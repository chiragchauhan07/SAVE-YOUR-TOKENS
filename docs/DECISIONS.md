...
`BLUEPRINT_LOG_LEVEL=INFO` or `DEBUG` for development visibility —
...
**Consequence.** `pip install blueprint` (no extras) still gets a
dependency-free analysis engine and generator; `pip install
blueprint[mcp]` (or `[dev]`) is required to run the MCP server.
...
tool already has filesystem access to `.blueprint/`) — directly the kind
...
**Consequence.** `blueprint-mcp` (the installed console script) and
...
**Decision.** `.blueprint/.cache/cache.json` stores only fingerprints and
...
## D-053 — The "Save your Tokens" → Blueprint rebrand touches only user-facing surface

**Context.** The project reached a mature, production-ready state (Phases
1–6 complete) and was rebranded from "Save your Tokens" to its canonical
name, **Blueprint**. A rename this broad has to decide, deliberately,
where the line sits between "cosmetic branding" and "breaking change" —
naively renaming everything (every internal module, every class, the
actual GitHub repository) would have been a much larger, riskier change
than the rebrand itself required.

**Decision.** Exactly four things changed:

1. The default Knowledge Base directory: `.ai-context/` → `.blueprint/`
   (`generator/output.py::DEFAULT_OUTPUT_DIRNAME`). A pre-existing legacy
   directory at the default location is renamed to `.blueprint/` in place
   automatically, the first time anything resolves the default output
   directory (`default_output_dir()`) — a lossless filesystem rename, not
   a regeneration. Never applied to a caller-supplied custom
   `--output`/`output_dir`.
2. Console script names: `save-your-tokens` → `blueprint`,
   `save-your-tokens-mcp` → `blueprint-mcp` (`pyproject.toml`
   `[project.scripts]`). Both old names stay installed, pointing at the
   exact same `cli:main`/`server:main` entry functions — not a second
   implementation — and print a one-time deprecation notice to stderr,
   detected via `sys.argv[0]` at the top of `main()` (`cli.py`,
   `mcp_server/server.py`).
3. One environment variable: `SAVE_YOUR_TOKENS_LOG_LEVEL` →
   `BLUEPRINT_LOG_LEVEL`, with the old name read as a fallback (and a
   logged warning) only when the new one isn't set
   (`mcp_server/server.py::_resolve_log_level_name`).
4. The `pyproject.toml` package name: `save-your-tokens` → `blueprint`,
   plus the FastMCP server identity (`FastMCP("blueprint")`) and every
   user-facing string (README, docs, CLI help text, the "Generated by
   Blueprint" Knowledge Base footer).

Everything else — every module path, package name, class name, function
name, the internal dependency rules (D-028, D-041, D-044, D-051), the
`.cache/cache.json` filename inside the KB directory, every `D-0XX`
decision's underlying reasoning — is unchanged. The actual GitHub
repository (`chiragchauhan07/SAVE-YOUR-TOKENS`) was deliberately left
unrenamed: renaming a shared, hosted resource is a different order of
action from renaming strings and directories in the codebase, and nothing
about this rebrand required it.

**Why.** The four items above are the only things a *user* (as opposed to
a contributor reading source) actually observes: what they type, what
directory appears in their repository, what environment variable they
set, what shows up in search/installation. Renaming internal module
names (`analyzer/`, `mcp_server/`, `incremental/`, `cli.py`, `server.py`)
would have touched dozens of import statements across the whole codebase
for zero user-facing benefit — pure risk (of breaking something in a
mechanical rename across ~30 files) for no reward, which is exactly the
kind of change `CLAUDE.md`'s engineering discipline (and this phase's own
explicit instruction to prioritise maintainability over unnecessary
internal churn) rules out.

**Consequence.** Every existing installation, script, CI pipeline or MCP
client configuration referencing `save-your-tokens`/`save-your-tokens-mcp`
/`SAVE_YOUR_TOKENS_LOG_LEVEL`/`.ai-context/` keeps working unmodified
through the deprecated aliases and the automatic directory migration —
nothing breaks on upgrade, and the deprecation notices are the only
signal telling an existing user to switch to the new names on their own
schedule. The engine's dependency graph, correctness guarantees and test
suite required zero structural changes to support the rebrand — a strong
signal the branding/implementation boundary was already clean before this
phase, not something this phase had to create.
